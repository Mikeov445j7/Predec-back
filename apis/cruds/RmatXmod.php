...
function A($conexionBD, $id_proyec, $id, $cantXmod){
    ...
    $sqlPredec = mysqli_set_charset($conexionBD, "utf8"); 
    $sqlPredec = mysqli_query($conexionBD,
               "SELECT materiales.descripcion, rel_actv_mat.id_rel, rel_actv_mat.cant_por_acti, materiales.unidad, materiales.PU, materiales.id_mat
        ...
        if(mysqli_num_rows($sqlPredec) > 0){
               while($row2 = mysqli_fetch_array($sqlPredec)){
                    $pu = intersecc($conexionBD, 'pu_us_mat', $id_proyec, $row2[5], $row2[4]);
                    $parcial = $row2[2] * $pu;
            ...
            $actividad -> insumo = 'SIN MATERIALES';
            array_push($materiales, $actividad);
