...
function actiXmods($conexionBD, $id_proyec, $id_modulo){
    ...
    "SELECT rel_actv_modulo.id_modulo AS modulos, 
            ...
            rel_actv_mo.id_mo As idManoObra, 
            ...
            mano_obra.id_mo AS idInsumo, 
            mano_obra.jerarquia AS jerarquia
    ...
    AND rel_actv_mo.id_actividad = actividades.id_actividad 
    AND rel_actv_mo.id_mo = mano_obra.id_mo 
    GROUP by rel_actv_mo.id_mo ORDER by mano_obra.jerarquia ASC"
    );

    if(mysqli_num_rows($sqlPredec) > 0){
        while($row2 = mysqli_fetch_array($sqlPredec)){
            $pu = intersecc($conexionBD, 'pu_us_mo', $id_proyec, $row2[13], $row2[9]);
            $costoT = $row2[12] * $pu;
            $dataActv = [
                "modulo" => $row2[0],
                "cantXmodulo" => $row2[1],
                ...
                "insumo" => $row2[7],
                "cantXactiv"  => $row2[8],
                "pUnitario"  => intersecc($conexionBD, 'pu_us_mo', $id_proyec, $row2[13], $row2[9]),
                "sumatoria" => $row2[10],
